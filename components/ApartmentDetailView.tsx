...
const ApartmentDetailView: React.FC<ApartmentDetailViewProps> = ({ apartment, theme, onBack, onSave, integrationUrl }) => {
  const [data, setData] = useState<Apartment>(() => {
    ...
    if (!initial.beds || initial.beds.length === 0) {
      if (initial.floor === 200) {
        initial.beds = [
          { type: 'Casal', baseStatus: 'Nova', mattressStatus: 'Novo', hasSkirt: false },
          { type: 'Casal', baseStatus: 'Nova', mattressStatus: 'Novo', hasSkirt: false }
        ];
      } else {
        initial.beds = [
          { type: 'Casal', baseStatus: 'Nova', mattressStatus: 'Novo', hasSkirt: false },
          { type: 'Solteiro', baseStatus: 'Nova', mattressStatus: 'Novo', hasSkirt: false },
          { type: 'Solteiro', baseStatus: 'Nova', mattressStatus: 'Novo', hasSkirt: false }
        ];
      }
    }
  ...
  const getStatusBtnClass = (current: any, target: any, activeBg: string) => {
    return `flex-1 py-4 rounded-2xl text-[10px] font-black border-2 transition-all ${
      current === target ? `${activeBg} text-white border-transparent shadow-md` : 'bg-slate-50 border-slate-50 text-slate-300'
    }`;
  };
    ...
    <div className="bg-slate-50 min-h-screen pb-40 font-sans text-slate-900">
      ...
      <div className="sticky top-0 z-[110] bg-white border-b border-slate-100 px-4 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Vistoria Mobile-First</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-6 space-y-5 max-w-lg mx-auto">
        ...
        <section className="bg-white p-6 rounded-[2.5rem] border border-slate-50 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {['Granito', 'Madeira', 'Cerâmica'].map(t => (
              <button key={t} onClick={() => updateField('pisoType', t)} className={getStatusBtnClass(data.pisoType, t, 'bg-blue-600')}>{t}</button>
            ))}
          </div>
          <div className="grid grid-cols-3 gap-2">
        ...
        <section className="bg-white p-6 rounded-[2.5rem] border border-slate-50 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {['Reformado', 'Antigo'].map(t => (
              <button key={t} onClick={() => updateField('banheiroType', t)} className={getStatusBtnClass(data.banheiroType, t, 'bg-indigo-600')}>{t}</button>
            ))}
          </div>
          <div className="grid grid-cols-2 gap-2">
        ...
        <section className="bg-white p-6 rounded-[2.5rem] border border-slate-50 shadow-sm space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {['Midea', 'LG', 'Grey'].map(b => (
                <button key={b} onClick={() => updateField('acBrand', b)} className={getStatusBtnClass(data.acBrand, b, 'bg-cyan-600')}>{b}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {['LG', 'Samsung', 'Philco', 'Smart Roku', 'Toshiba'].map(b => (
                <button key={b} onClick={() => updateField('tvBrand', b)} className={getStatusBtnClass(data.tvBrand, b, 'bg-slate-800')}>{b}</button>
              ))}
            </div>
          </div>
        </section>
        ...
        <section className="bg-white p-6 rounded-[2.5rem] border border-slate-50 shadow-sm space-y-5">
          ...
          <div className="space-y-4">
              ...
              {data.temEspelhoCorpo && (
                <div className="grid grid-cols-3 gap-1">
                  {['Bom estado', 'Manchado', 'Danificado'].map(s => (
                    <button key={s} onClick={() => updateField('espelhoCorpoStatus', s)} className={`py-2 rounded-lg text-[8px] font-black border-2 ${data.espelhoCorpoStatus === s ? 'bg-slate-800 text-white border-transparent' : 'bg-white text-slate-400 border-slate-100'}`}>{s.toUpperCase()}</button>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="p-5 bg-slate-50 rounded-[2rem] space-y-4">
              ...
              {data.temCortina && (
                <div className="space-y-3 animate-in fade-in">
                  <div className="grid grid-cols-2 gap-2">
                    <button onClick={() => updateField('cortinaStatus', 'Nova')} className={getStatusBtnClass(data.cortinaStatus, 'Nova', 'bg-emerald-500')}>NOVA</button>
                    <button onClick={() => updateField('cortinaStatus', 'Antiga')} className={getStatusBtnClass(data.cortinaStatus, 'Antiga', 'bg-amber-400 !text-slate-900')}>ANTIGA</button>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
            ...
            <div className="grid grid-cols-2 gap-2">
               <button onClick={() => updateField('temCofre', !data.temCofre)} className={`p-5 rounded-2xl border-2 flex flex-col items-center justify-center transition-all ${data.temCofre ? 'bg-white border-slate-50 text-slate-400' : 'bg-white border-slate-50 text-slate-400'}`}>
                 <span className="text-[7px] font-black uppercase mb-1 tracking-widest">Possui Cofre?</span>
                 <span className="text-xs font-black">{data.temCofre ? 'SIM' : 'NÃO'}</span>
               </button>
               <button onClick={() => updateField('temPortaControle', !data.temPortaControle)} className={`p-5 rounded-2xl border-2 flex flex-col items-center justify-center transition-all ${data.temPortaControle ? 'bg-white border-slate-50 text-slate-400' : 'bg-white border-slate-50 text-slate-400'}`}>
                 <span className="text-[7px] font-black uppercase mb-1 tracking-widest">Porta-Controles?</span>
                 <span className="text-xs font-black">{data.temPortaControle ? 'SIM' : 'NÃO'}</span>
               </button>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white p-6 rounded-[2.5rem] border border-slate-50 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {['Arandela', 'Vidro', 'Quadrado'].map(t => (
              <button key={t} onClick={() => updateField('luminariaType', t)} className={getStatusBtnClass(data.luminariaType, t, 'bg-yellow-600')}>{t}</button>
            ))}
          </div>
          {data.luminariaType === 'Quadrado' && (
             <div className="p-4 bg-slate-50 rounded-2xl space-y-3 animate-in fade-in">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Cor da Luminária Quadrada:</p>
                <div className="grid grid-cols-2 gap-2">
                   {['Branco', 'Preto'].map(color => (
                     <button key={color} onClick={() => updateField('luminariaColor', color)} className={`py-4 rounded-xl text-[10px] font-black border-2 transition-all ${data.luminariaColor === color ? 'bg-white border-slate-200 text-slate-800 shadow-sm' : 'bg-white border-transparent text-slate-300'}`}>{color.toUpperCase()}</button>
                   ))}
                </div>
             </div>
          )}
          ...
          {(data.beds || []).map((bed, idx) => (
            <section key={idx} className="bg-white p-6 rounded-[2.5rem] border border-slate-50 shadow-sm space-y-6">
              <div className="flex justify-between items-center">
                ...
                <div className="bg-emerald-50 text-emerald-500 px-3 py-1 rounded-full border border-emerald-100 text-[8px] font-black uppercase tracking-widest">
                  {bed.type}
                </div>
              </div>
              ...
              <div className="space-y-3">
                <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest ml-1">Base da Cama:</label>
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => updateBed(idx, { baseStatus: 'Nova', baseColor: undefined })} className={getStatusBtnClass(bed.baseStatus, 'Nova', 'bg-emerald-500')}>NOVA</button>
              ...
              {/* Colchão */}
              <div className="space-y-3">
                <label className="text-[9px] font-black text-slate-300 uppercase tracking-widest ml-1">Colchão:</label>
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => updateBed(idx, { mattressStatus: 'Novo', mattressColor: undefined })} className={getStatusBtnClass(bed.mattressStatus, 'Novo', 'bg-emerald-500')}>NOVO</button>
        ...
        {/* RELATO DE DEFEITOS / MANUTENÇÃO */}
        <section className="bg-white p-6 rounded-[2.5rem] border border-slate-50 shadow-xl space-y-4">
          <SectionTitle icon={Paperclip} title="Relato de Defeitos / Manutenção" color="text-rose-500" />
          <textarea 
            value={newDefectText} 
          ...
          <button 
            ...
            className="w-full bg-rose-500 text-white py-4 rounded-2xl font-black text-xs flex items-center justify-center space-x-3 active:scale-95 transition-all shadow-lg uppercase tracking-widest"
          >
            <ImageIcon size={20} />
                ...
                <div className="flex items-start justify-between gap-4">
                   <div className="flex items-start space-x-3 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        {defect.driveLink && defect.driveLink !== 'pendente' && (
                          <div className="mt-2 flex items-center space-x-2">
                             <span className="text-[7px] font-black text-emerald-500 bg-emerald-50 px-1.5 py-0.5 rounded uppercase flex items-center"><ExternalLink size={8} className="mr-1"/> No Drive</span>
                          </div>
                        )}
                      </div>
                   </div>
      ...
      <div className="fixed bottom-8 left-0 right-0 px-6 z-[120]">
        <button 
          ...
          disabled={isSaving}
          className="w-full bg-slate-900 text-white py-5 rounded-[2.2rem] font-black text-sm shadow-2xl flex items-center justify-center space-x-3 active:scale-95 transition-all hover:bg-slate-800"
        >
          {isSaving ? (
          ...
          <span>{isSaving ? 'PROCESSANDO...' : 'SALVAR E SAIR'}</span>
        </button>
      </div>
    </div>
  );
